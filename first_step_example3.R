...
cat("Reading data...\n")
tr <- read_csv("/lustre1/projects/fs_ma8701_1/avito/train.csv") 
...
cat("Parsing text...\n")
cnt <- seq(1, 7, 1)
testrmse2 <- vector("numeric", length(cnt))
...
vect <- create_vocabulary(it, ngram = c(1, 1), stopwords = stopwords("ru")) %>%
  prune_vocabulary(term_count_min = cnt[k], doc_proportion_max = 0.4, vocab_term_max = 12500) %>% 
  vocab_vectorizer()
...
dmattr=dmat[randtrain,]
dmatval=dmat[randvalid,]
dmattest=dmat[randtest,]
...
{ 
  print(i)
  thislambda=lambdas[i]
  yhats=predict(fit2,newx=dmatval,type="response",s=thislambda)
  rmse[i]=sqrt(mean((Yval-yhats)^2))
}
plot(lambdas,rmse)
